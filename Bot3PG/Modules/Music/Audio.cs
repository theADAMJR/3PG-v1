...
namespace Bot3PG.Modules.Music
{
    public sealed class Music : CommandBase
    {
        ...
        public async Task Play([Remainder]string search) => await ReplyAsync(await AudioService.JoinOrPlayAsync((SocketGuildUser)Context.User, Context.Channel as SocketTextChannel, Context.Guild.Id, search));
        ...
        [RequireUserPermission(GuildPermission.Speak)]
        public async Task Volume(int volume = 0) => await ReplyAsync(await AudioService.VolumeAsync(Context.Guild.Id, volume));
